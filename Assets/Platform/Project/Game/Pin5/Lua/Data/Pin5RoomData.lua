...
local this = Pin5RoomData


--判断是否开局了，牌局是否开始
function Pin5RoomData.IsGameStarted()
...
--开局重置的数据
function Pin5RoomData.StartGameReset()
    this.Reset()
    for i = 1, #this.playerDatas do
...
--获取自己的Data
function Pin5RoomData.GetSelfData()
...
function Pin5RoomData.CheckHaveSelfData()
...
--根据玩家id移除玩家
function Pin5RoomData.RemovePlayer(playerId)
...
--获取玩家第五张牌是否翻开
function Pin5RoomData:GetSelfHandCardFiveFlip()
    local selfData = Pin5RoomData.GetSelfData()
    return not selfData:GetHandCardFiveFlip()
end

--获取自己是否未准备
function Pin5RoomData.GetSelfIsNoReady()
    if this.GetSelfData().state == Pin5PlayerState.WAITING or this.GetSelfData().state == Pin5PlayerState.NO_READY then
        return true
    end
    return false
end
...
--主玩家是否观战状态
function Pin5RoomData.GetSelfIsWaiting()
    return this.GetSelfData().state == Pin5PlayerState.WAITING
end

--主玩家是否可以离开房间
function Pin5RoomData.GetSelfIsExitRoom()
...
--获取当前游戏是否在游戏中的状态
function Pin5RoomData.GetIsGaming()
...
function Pin5RoomData.GetPlayerDataById(playerId)
...
function Pin5RoomData.GetPlayerIndexById(playerId)
...
--获取玩家的性别
function Pin5RoomData.GetPlayerGender(playerId)
    local sex = 1
    for i = 1, #this.playerDatas do
        local playerData = this.playerDatas[i]
...
--获取非观战的玩家
function Pin5RoomData.GetReadyPlayer()
    local tab = {}
    for i = 1, #this.playerDatas do
        if this.playerDatas[i].state ~= Pin5PlayerState.LookOn then
...
--根据座位号获取玩家信息
function Pin5RoomData.GetPlayerBySeatNumber(seatId)
    for i = 1, #this.playerDatas do
...
--根据服务器座位号，获取本地座位号
function Pin5RoomData.GetIndexBySeatNumber(num)
...
--获取除去自己以外的玩家的座位id
function Pin5RoomData.GetReadyOtherPlayerDatas()
    local tab = {}
...
function Pin5RoomData.GetPlayerItemById(id)
    local playerData = this.GetPlayerDataById(id)
    if playerData == nil then
        LogWarn(">> Pin5RoomData.GetPlayerItemById > playerData is nil")
        return
    end
    if playerData.item == nil then
        LogWarn(">> Pin5RoomData.GetPlayerItemById > playerData.item is nil")
        return
    end
...
--主玩家是否是房主
function Pin5RoomData.MainIsOwner()
    return this.mainId == this.owner
end

--主玩家是否是庄家
function Pin5RoomData.MainIsBanker()
    return this.mainId == this.BankerPlayerId
end

--是否大厅房间
function Pin5RoomData.IsLobbyRoom()
...
--是否俱乐部房间
function Pin5RoomData.IsClubRoom()
    if IsNil(this.roomData) then
        return false
    end
    local roomType = this.roomData.roomType
...
--是否亲友圈大厅房间
function Pin5RoomData.IsTeaRoom()
    if IsNil(this.roomData) then
        return false
    end
    local roomType = this.roomData.roomType
...
--是否联盟房间
function Pin5RoomData.IsUnionRoom()
    if IsNil(this.roomData) then
        return false
    end
    local roomType = this.roomData.roomType
...
--是否房卡场
function Pin5RoomData.IsFangKaGame()
    if IsNil(this.roomData) then
        return false
    end
...
--是否是金币场
function Pin5RoomData.IsGoldGame()
    if IsNil(this.roomData) then
        return false
    end
    local moneyType = this.roomData.moneyType
...
--是否房卡流程
function Pin5RoomData.IsFangKaFlow()
...
---更新获奖记录
function Pin5RoomData.UpdateRewardRecord(lastReward)
    this.RewardRecord = lastReward
end

---更新奖池显示
function Pin5RoomData.UpdateAwardPoolCoinNum(awardPoolNum)
    if Pin5RoomData.awardPoolCoinNum ~= awardPoolNum then
        Pin5RoomData.awardPoolCoinNum = awardPoolNum
        if Pin5RoomPanel then
            Pin5RoomPanel.UpdateAwardPoolText(awardPoolNum)
        end
    end
end
...
function Pin5RoomData.CheckPlayNumJudgeIsObserver()
    return #Pin5RoomData.playerDatas == Pin5RoomData.manCount
end

function Pin5RoomData.JudgeIsObserver(playerList)
    ...
    SendMsg(Pin5Action.Pin5HideOperate)
    return this.isObserver
end

---判断座位是否坐满
---@return boolean true坐满，false没有
function Pin5RoomData.IsFullOfSeat()
    return #Pin5RoomData.playerDatas == Pin5RoomData.manCount
end

---@return boolean true为观战者，false为玩家
function Pin5RoomData.IsObserver()
    --LogError("Handle IsObserver", this.isObserver)
    return this.isObserver
end

function Pin5RoomData.JudgeSitDownPlayer(data)
    local userId = data.userId
    --LogError("<color=aqua>data</color>", data)
    if userId == UserData.userId and this.IsObserver() and data.seatNum > 0 then
        this.isObserver = false
        --LogError("<color=aqua>Pin5ObserverSitDown</color>")
        Pin5RoomCtrl.ResetAllPlayerUI()
        --SendMsg(Pin5Action.Pin5ObserverSitDown)
    end
end