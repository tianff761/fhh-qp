CreateEqsRoomPanel = ClassLuaComponent("CreateEqsRoomPanel")
CreateEqsRoomPanel.Instance = nil

--
local CELL_SIZE_1 = Vector2(562, 180)
--
...
local this = CreateEqsRoomPanel

function CreateEqsRoomPanel:Init()
    --大厅玩法数据
...
--UI初始化
function CreateEqsRoomPanel:Awake()
    this = self
    self:Init()
    -----------------------------------------------------------------------
    --玩法菜单按钮
...
--当面板开启开启时
function CreateEqsRoomPanel:OnOpened(fromType, funcType, args)
    LogError(">> CreateEqsRoomPanel > OnOpened > ", fromType, funcType)
    CreateEqsRoomPanel.Instance = self
...
--当面板关闭时调用
function CreateEqsRoomPanel:OnClosed()
    CreateEqsRoomPanel.Instance = nil
...
------------------------------------------------------------------
--初始外部菜单
function CreateEqsRoomPanel.InitExternalMenu(menuToggleDict)
...
------------------------------------------------------------------
--创建
function CreateEqsRoomPanel.OnCreateBtnClick()
...
--修改
function CreateEqsRoomPanel.OnModifyBtnClick()
    if os.time() - this.createClickTime < 3 then
        Toast.Show("请不要频繁操作")
        return
    end
...
--创建房间提示处理
function CreateEqsRoomPanel.OnCreateAlert()
    this.HandleCreateRoom()
end

--俱乐部和茶馆的保存
function CreateEqsRoomPanel.OnSaveBtnClick()
...
--保存一键配置提示处理
function CreateEqsRoomPanel.OnSaveAlert()
...
--俱乐部的删除
function CreateEqsRoomPanel.OnDeleteBtnClick()
    if this.roomType == RoomType.Club then
...
--删除配置提示处理
function CreateEqsRoomPanel.OnDeleteAlert()
    if this.roomType == RoomType.Club then
...
--高级设置按钮
function CreateEqsRoomPanel.OnAdvancedBtnClick()
    PanelManager.Open(PanelConfig.CreateRoomAdvanced, this.advancedData, EqsConfig.DiFenConfig, EqsConfig.DiFenNameConfig)
end

--菜单按钮点击
function CreateEqsRoomPanel.OnPlayWayMenuValueChanged(isOn, listener)
    if isOn then
        this.SavePlayWayConfigData()
        local dataIndex = tonumber(listener.name)

        local playWayConfigData = EqsRule.CreateRoomConfig[dataIndex]
...
local bdPer = nil
--处理创建房间
function CreateEqsRoomPanel.HandleCreateRoom()
    tempRuleDatas = nil
    ...
    if this.roomType == RoomType.Lobby then
        BaseTcpApi.SendCreateRoom(GameType.ErQiShi, tempRuleDatas, tempPlayerTotal, tempGameTotal, this.roomType,
...
--检测定位
function CreateEqsRoomPanel.OnCheckGpsCompleted()
...
--检测定位提示处理
function CreateEqsRoomPanel.OnGpsAlertCallback()
    AppPlatformHelper.OpenDeviceSetting()
end

--检测按钮显示
function CreateEqsRoomPanel.CheckButtonDisplay()
...
--
--处理传递参数数据
function CreateEqsRoomPanel.CheckArgsData(fromType, moneyType, args)
    --处理打开面板的来源
    ...
    elseif this.roomType == RoomType.Tea then
        ...
        if this.isModify then
            --表示修改
            this.lastPlayWayType = EqsConfig.GetPlayWayTypeByName(this.args.playWayName)
...
--更新配置数据，俱乐部的配置数据更新
function CreateEqsRoomPanel.CheckAndUpdateConfigData()
    --先清除当前存储的配置数据
    ...
    length = #EqsRule.CreateRoomConfig
    for i = 1, length do
        playWayConfigData = EqsRule.CreateRoomConfig[i]
        --
...
--清理显示项相关
function CreateEqsRoomPanel.ClearDisplay()
    this.gameTotalItems = {}
    this.huPaiSuanFaItems = {}
    this.limitItems = {}
...
--更新配置数据的显示
function CreateEqsRoomPanel.UpdatePlayWayConfigDataDisplay()
    this.inited = false
    this.ClearDisplay()
    ...
    Log(">> CreateEqsRoomPanel.UpdatePlayWayConfigDataDisplay > ", playWayRuleData)
...
--创建一个规则显示项
function CreateEqsRoomPanel.CreatePlayWayRuleItem(ruleGroupIndex, playWayRuleGroupConfigData)
    local item = {}
...
--设置一个规则显示项数据
function CreateEqsRoomPanel.SetPlayWayRuleItem(playWayRuleItem, playWayRuleGroupConfigData, playWayRuleData)
    ...
    for i = 1, #ruleGroups do
        ...
        elseif ruleGroupConfigType == EqsRule.RuleGroupConfigType.HuShu then
            itemMaxCol = 1
...
--处理规则分组
function CreateEqsRoomPanel.HandleRuleGroup(ruleGroups, ruleConfigData, groupName)
...
--创建规则组显示项
function CreateEqsRoomPanel.CreatePlayWayRuleGroupItem(index, parent)
    local item = {}
...
--设置规则组数据
function CreateEqsRoomPanel.SetPlayWayRuleGroupItem(ruleGroupItem, ruleGroup, playWayRuleData)
        ...
        if ruleConfigData.data.group ~= 0 then
            ...
            if item == nil then
                ...
                elseif ruleConfigData.data.type == EqsRuleType.Desc then
                    item = this.CreatePlayWayRuleConfigDescItem(this.textItemPrefab, ruleGroupItem.transform,
                        ruleConfigData.name)
...
--获取ToggleGroup组件
function CreateEqsRoomPanel.GetToggleGroupItem(type, group)
...
--创建具体的一个规则
function CreateEqsRoomPanel.CreatePlayWayRuleConfigItem(prefab, parent, order)
    local item = {}
    --用于标识是否激活
    item.isActive = false
    --用于事件
...
--创建可输入的规则Item
function CreateEqsRoomPanel.CreatePlayWayRuleConfigInputItem(prefab, parent, order)
    local item = {}
    --用于标识是否激活
    item.isActive = false
    --用于事件
...
--创建文本显示项
function CreateEqsRoomPanel.CreatePlayWayRuleConfigDescItem(prefab, parent, order)
    local item = {}
    --用于标识是否激活
    item.isActive = false
    --用于事件
...
--设置具体规则数据
function CreateEqsRoomPanel.SetPlayWayRuleConfigItem(ruleConfigItem, ruleConfigData, playWayRuleData)
    --存储数据，用于获取
...
function CreateEqsRoomPanel.UpdateToggleStatus(ruleConfigItem, isOn)
...
--处理Toggle选项的选择状态
function CreateEqsRoomPanel.UpdateToggleSelectedStatus(toggle)
...
--处理选项值改变
function CreateEqsRoomPanel.HandleToggleValueChanged(go)
...
--选项值改变
function CreateEqsRoomPanel.OnToggleValueChanged(isOn, listener)
...
--胡牌算法
function CreateEqsRoomPanel.OnHuPaiSuanFaChanged(isOn, listener)
    if isOn then
        if not this.inited then
            return
        end
...
--更新胡牌算法文本描述显示
function CreateEqsRoomPanel.UpdateHuPaiSuanFaDisplay()
    local item = nil
    local txt = nil
...
--局数选择
function CreateEqsRoomPanel.OnConsumeCardsChanged(isOn, listener)
    if not this.inited then
        return
    end
    this.HandleToggleValueChanged(listener.gameObject)
...
--更新钻石消耗显示
function CreateEqsRoomPanel.UpdateConsumeDisplay()
    local item = nil
    local cards = 0
...
--
--检测大厅的玩法缓存数据
function CreateEqsRoomPanel.CheckLobbyPlayWayData()
...
--保存到本地存储中
function CreateEqsRoomPanel.SaveLobbyPlayWayConfigData()
    if this.lobbyPlayWayData ~= nil then
        local temp = {}
...
--保存配置数据
function CreateEqsRoomPanel.SavePlayWayConfigData()
    if this.playWayConfigData == nil then
        return nil
    end
...
function CreateEqsRoomPanel.GetPlayWayRuleDataAtUI()
    if this.playWayConfigData == nil then
        return {}
    end
    ...
    --     end
    -- end
    LogError(">> CreateEqsRoomPanel.GetPlayWayRuleDataAtUI > ", playWayRuleData)
    return playWayRuleData
end

--检测规则组显示对象
function CreateEqsRoomPanel.CheckRuleGroupItem(ruleGroupItem, playWayRuleData, includeRules)
...
--获取规则的数据对象，用于服务器交互
function CreateEqsRoomPanel.CheckAndSetRuleData(playWayRuleData, ruleConfigData, selected, inputValue)
    if ruleConfigData == nil then
        return
    end
...
--更新创建房间高级设置
function CreateEqsRoomPanel.OnUpdateCreateRoomAdvanced(data)
