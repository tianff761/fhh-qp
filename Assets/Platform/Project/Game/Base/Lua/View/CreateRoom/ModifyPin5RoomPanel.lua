ModifyPin5RoomPanel = ClassLuaComponent("ModifyPin5RoomPanel")
ModifyPin5RoomPanel.Instance = nil

local this = ModifyPin5RoomPanel

function ModifyPin5RoomPanel:Init()
...
function ModifyPin5RoomPanel:Awake()
    this = self
    self:Init()
...
function ModifyPin5RoomPanel:OnOpened(fromType, funcType, args, isModify, playWayName, rules, advancedData)
...
--
function ModifyPin5RoomPanel:OnClosed()
...
---UI监听事件
function ModifyPin5RoomPanel:AddUIListenerEvent()
...
function ModifyPin5RoomPanel.InitExternalMenu(listToggles)
    if listToggles ~= nil and this.externalMenuItems == nil then
        local trans = listToggles[GameType.Pin5]
        if trans ~= nil then
...
function ModifyPin5RoomPanel.ButtonActiveCtrl()
    LogError("this.isModify", this.isModify)
...
--检测按钮显示
function ModifyPin5RoomPanel.CheckButtonDisplay()
...
function ModifyPin5RoomPanel.CheckAndUpdateConfigData()
...
function ModifyPin5RoomPanel.RevertLastModifyDisplay()
...
function ModifyPin5RoomPanel.GetDropdownIndexOfConfig(config, Pin5RuleType)
    local index = 0
    for i = 1, #config do
...
function ModifyPin5RoomPanel.CheckLobbyPlayWayData()
...
--玩家总数
function ModifyPin5RoomPanel.OnPlayerTotalDropDown(value)
    this.UpdateConsumeDisplay()
end
--底分
function ModifyPin5RoomPanel.OnBaseCoreDropDown(value)
    --Log('OnBaseCoreDropDown', value)
    --this.CheckRuleValue(Pin5RuleType.BaseScore, value)
end

--局数
function ModifyPin5RoomPanel.OnGameTotalDropDown(value)
    this.UpdateConsumeDisplay()
end

--开始模式
function ModifyPin5RoomPanel.OnStartModelDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.StartModel, value)
end

function ModifyPin5RoomPanel.OnPayDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.PayType, value)
end

--推注
function ModifyPin5RoomPanel.OnTuiZhuDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.TuiZhu, value)
end
--最大抢庄
function ModifyPin5RoomPanel.OnMaxQiangDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.MaxQiangZhuang, value)
end

--癞子
function ModifyPin5RoomPanel.OnLaiZiDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.LaiZi, value)
end
--翻倍规则
function ModifyPin5RoomPanel.OnFanBeiDropDown(value)
    --this.CheckRuleValue(Pin5RuleType.FanBeiRule, value)
    this.UpdateNiuCardTxt()
end

--通过Dropdown索引获取值
function ModifyPin5RoomPanel.CheckRuleValue(ruleType, index)
    local tempValue = 0
...
--检测输入值
function ModifyPin5RoomPanel.CheckInputValue(value)
...
--纠正Dropdown中的索引
function ModifyPin5RoomPanel.ExchangeDropValueByRule(ruleType, ruleValue)
    if ruleType == Pin5RuleType.TuiZhu then
...
--特殊牌型按钮
function ModifyPin5RoomPanel.OnSpecialCardBtnClick()
...
--高级选项按钮
function ModifyPin5RoomPanel.OnHighOptionBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
...
function ModifyPin5RoomPanel.OnCloseOptionBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
    UIUtil.SetActive(this.optionGroupNode, false)
...
function ModifyPin5RoomPanel.OnCloseSpecialBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
    UIUtil.SetActive(this.optionGroupNode, false)
...
--判断牛型是否全选
function ModifyPin5RoomPanel.CheckSpecialIsAllSlected()
...
--判断高级选项是否全选
function ModifyPin5RoomPanel.CheckOptionAllSlected()
    local isAllSlected = true
    for i = 1, 4 do
...
--特殊类型选择
function ModifyPin5RoomPanel.OnNiuTypeValueChanged(isOn, listener)
...
--特殊牌型的全选
function ModifyPin5RoomPanel.AllNiuTypeValueChanged(isOn, listener)
...
--高级选项的选择
function ModifyPin5RoomPanel.OnOptionValueChanged(isOn, listener)
    if this.isOpend then
        this.CheckOptionAllSlected()
        this.UpdateOptionTxt()
    end
...
--高级选项的全选
function ModifyPin5RoomPanel.AllOptionValueChanged(isOn, listener)
    if this.isNotTogglePress then
        return
    end
    for i = 1, 4 do
...
--更新高级选项文本
function ModifyPin5RoomPanel.UpdateOptionTxt()
    local tempStr = ""
    for i = 1, 4 do
...
--更新牛型显示文本
function ModifyPin5RoomPanel.UpdateNiuCardTxt()
    local tempStr = ""
...
--保存到本地存储中
function ModifyPin5RoomPanel.SaveLobbyPlayWayConfigData()
    if this.lobbyPlayWayData ~= nil then
        local temp = {}
...
--保存配置数据
function ModifyPin5RoomPanel.SavePlayWayConfigData()
...
--无规则时设置默认规则界面
function ModifyPin5RoomPanel.SetDefaultRuleUI()
...
--获取当前选择的UI上的默认字段
function ModifyPin5RoomPanel.GetPlayWayRuleDataAtUI()
    --玩法规则对象
    ...
    LogError(">> ModifyPin5RoomPanel.GetPlayWayRuleDataAtUI > ", playWayRuleData)
    return playWayRuleData
end

function ModifyPin5RoomPanel.GetTeShuCardType(type, toggle, str)
...
function ModifyPin5RoomPanel.GetHighOptionCardType(type, toggle, str)
    local isOne = this.GetToggleRuleValue(toggle.toggle.isOn) == 1
    if isOne then
        if string.IsNullOrEmpty(str) then
...
function ModifyPin5RoomPanel.GetToggleRuleValue(isOn)
    if isOn then
        return 1
    else
        return 0
...
function ModifyPin5RoomPanel.SetRuleUI(rules)
    if rules == nil then
...
--获取规则值
function ModifyPin5RoomPanel.GetRuleValue(rules, type)
...
function ModifyPin5RoomPanel.UpdateDropDown(ruleValue)
    local optionDatas = {}
    for i = 1, 3 do
...
function ModifyPin5RoomPanel.OnCreateBtnClick()
...
function ModifyPin5RoomPanel.OnCreateAlert()
    this.HandleCreateRoom()
end

--高级设置按钮
function ModifyPin5RoomPanel.OnAdvancedBtnClick()
...
local wins = ""
local consts = ""
function ModifyPin5RoomPanel.HandleCreateRoom()
    tempRuleDatas = nil
    --只有大厅创建房间才进行规则存储
...
--保存一键配置
function ModifyPin5RoomPanel.OnSaveBtnClick()
    if this.roomType == RoomType.Club then
        Alert.Prompt("确定保存一键开房配置信息？", this.OnSaveAlert)
    end
end

function ModifyPin5RoomPanel.OnSaveAlert()
...
--删除一键配置
function ModifyPin5RoomPanel.OnDeleteBtnClick()
    if this.roomType == RoomType.Club then
        Alert.Prompt("确定删除一键开房配置？", this.OnDeleteAlert)
    end
end

function ModifyPin5RoomPanel.OnDeleteAlert()
    ClubData.SendRemoveYjpzRule(GameType.Pin5, this.lastPlayWayType)
end


--菜单按钮点击
function ModifyPin5RoomPanel.OnPlayWayMenuValueChanged(isOn, listener)
    if isOn then
...
--
function ModifyPin5RoomPanel.UpdateConsumeDisplay()
...
--获取label字体的颜色
function ModifyPin5RoomPanel.SetLabelColorTxt(toggle)
...
--更新创建房间高级设置
function ModifyPin5RoomPanel.OnUpdateCreateRoomAdvanced(data)
