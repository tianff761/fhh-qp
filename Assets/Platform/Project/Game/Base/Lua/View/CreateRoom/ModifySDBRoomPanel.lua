ModifySDBRoomPanel = ClassLuaComponent("ModifySDBRoomPanel")
ModifySDBRoomPanel.Instance = nil
...
local this = ModifySDBRoomPanel

function ModifySDBRoomPanel:Init()
    --大厅玩法数据
...
--UI初始化
function ModifySDBRoomPanel:Awake()
    this = self
    self:Init()
    -----------------------------------------------------------------------
    --玩法菜单按钮
...
--当面板开启开启时
function ModifySDBRoomPanel:OnOpened(fromType, funcType, args)
    ModifySDBRoomPanel.Instance = self
...
--当面板关闭时调用
function ModifySDBRoomPanel:OnClosed()
    ModifySDBRoomPanel.Instance = nil
...
--更新创建房间高级设置
function ModifySDBRoomPanel.OnUpdateCreateRoomAdvanced(data)
...
------------------------------------------------------------------
--初始外部菜单
function ModifySDBRoomPanel.InitExternalMenu(listToggles)
...
------------------------------------------------------------------
--创建
function ModifySDBRoomPanel.OnCreateBtnClick()
...
--创建房间提示处理
function ModifySDBRoomPanel.OnCreateAlert()
    this.HandleCreateRoom()
end

--高级设置按钮
function ModifySDBRoomPanel.OnAdvancedBtnClick()
    PanelManager.Open(PanelConfig.CreateRoomAdvanced, this.advancedData, SDB.DiFenConfig, SDB.DiFenNameConfig, GameType.SDB)
end

--亲友圈和茶馆的保存
function ModifySDBRoomPanel.OnSaveBtnClick()
    if this.roomType == RoomType.Club then
...
--保存一键配置提示处理
function ModifySDBRoomPanel.OnSaveAlert()
...
--亲友圈的删除
function ModifySDBRoomPanel.OnDeleteBtnClick()
    if this.roomType == RoomType.Club then
...
--删除配置提示处理
function ModifySDBRoomPanel.OnDeleteAlert()
    if this.roomType == RoomType.Club then
...
--菜单按钮点击
function ModifySDBRoomPanel.OnPlayWayMenuValueChanged(isOn, listener)
    if isOn then
        this.SavePlayWayConfigData()
...
--处理创建房间
function ModifySDBRoomPanel.HandleCreateRoom()
    tempRuleDatas = nil
...
--检测定位
function ModifySDBRoomPanel.OnCheckGpsCompleted()
...
--检测定位提示处理
function ModifySDBRoomPanel.OnGpsAlertCallback()
    AppPlatformHelper.OpenDeviceSetting()
end

--检测按钮显示
function ModifySDBRoomPanel.CheckButtonDisplay()
...
--
--处理传递参数数据
function ModifySDBRoomPanel.CheckArgsData(fromType, funcType, args)
    --处理打开面板的来源
...
--更新配置数据，亲友圈的配置数据更新
function ModifySDBRoomPanel.CheckAndUpdateConfigData()
    --先清除当前存储的配置数据
    this.playWayConfigData = nil
...
--清理显示项相关
function ModifySDBRoomPanel.ClearDisplay()
    this.fullSixStartItem = nil
    this.handStartItem = nil
    this.fullFourStartItem = nil
    this.isSlect4Ren = false
...
--更新配置数据的显示
function ModifySDBRoomPanel.UpdatePlayWayConfigDataDisplay()
    this.inited = false
    this.ClearDisplay()
...
--创建一个规则显示项
function ModifySDBRoomPanel.CreatePlayWayRuleItem(ruleGroupIndex, playWayRuleGroupConfigData)
    local item = {}
...
--设置一个规则显示项数据
function ModifySDBRoomPanel.SetPlayWayRuleItem(playWayRuleItem, playWayRuleGroupConfigData, playWayRuleData)
    --规则配置数据
...
--处理规则分组
function ModifySDBRoomPanel.HandleRuleGroup(ruleGroups, ruleConfigData, groupName)
...
--创建规则组显示项
function ModifySDBRoomPanel.CreatePlayWayRuleGroupItem(index, parent)
    local item = {}
...
--创建文本显示项
function ModifySDBRoomPanel.CreateDesTxtItem(index, parent)
    local item = {}
...
--设置规则组数据
function ModifySDBRoomPanel.SetPlayWayRuleGroupItem(ruleGroupItem, ruleGroup, playWayRuleData)
...
--获取ToggleGroup组件
function ModifySDBRoomPanel.GetToggleGroupItem(type, group)
...
--创建具体的一个规则
function ModifySDBRoomPanel.CreatePlayWayRuleConfigItem(prefab, parent, order)
    local item = {}
    --用于标识是否激活
    item.isActive = false
    --用于事件
...
--设置具体规则数据
function ModifySDBRoomPanel.SetPlayWayRuleConfigItem(ruleConfigItem, ruleConfigData, playWayRuleData)
    --存储数据，用于获取
...
function ModifySDBRoomPanel.UpdateToggleStatus(ruleConfigItem, isOn)
    local toggle = ruleConfigItem.toggle
    if isOn == nil then
        isOn = toggle.isOn
    end
...
--处理Toggle选项的选择状态
function ModifySDBRoomPanel.UpdateToggleSelectedStatus(toggle)
...
--处理选项值改变
function ModifySDBRoomPanel.HandleToggleValueChanged(go)
...
--选项值改变
function ModifySDBRoomPanel.OnToggleValueChanged(isOn, listener)
...
--局数选择
function ModifySDBRoomPanel.OnConsumeCardsChanged(isOn, listener)
    if not this.inited then
        return
    end
    this.HandleToggleValueChanged(listener.gameObject)
...
function ModifySDBRoomPanel.OnPlayerFourNumToggleChanged(isOn, listener)
    if not this.inited then
        return
    end
    if isOn then
...
function ModifySDBRoomPanel.OnPlayerSixNumToggleChanged(isOn, listener)
    if not this.inited then
        return
    end
    if isOn then
...
--更新房卡消耗显示
function ModifySDBRoomPanel.UpdateConsumeDisplay()
...
--
--检测大厅的玩法缓存数据
function ModifySDBRoomPanel.CheckLobbyPlayWayData()
...
--保存到本地存储中
function ModifySDBRoomPanel.SaveLobbyPlayWayConfigData()
    if this.lobbyPlayWayData ~= nil then
        local temp = {}
...
--保存配置数据
function ModifySDBRoomPanel.SavePlayWayConfigData()
    if this.playWayConfigData == nil then
        return nil
    end
...
function ModifySDBRoomPanel.GetPlayWayRuleDataAtUI()
    if this.playWayConfigData == nil then
        return {}
    end
...
--检测规则组显示对象
function ModifySDBRoomPanel.CheckRuleGroupItem(ruleGroupItem, playWayRuleData, includeRules)
...
--获取规则的数据对象，用于服务器交互
function ModifySDBRoomPanel.CheckAndSetRuleData(playWayRuleData, ruleConfigData, selected)
    if ruleConfigData == nil then
        return
    end
