ModifyLYCRoomPanel = ClassLuaComponent("ModifyLYCRoomPanel")
ModifyLYCRoomPanel.Instance = nil

local this = ModifyLYCRoomPanel

function ModifyLYCRoomPanel:Init()
    this.lastPlayWayType = LYCPlayType.MingPaiQiangZhuang  --最后一次玩法类型
...
function ModifyLYCRoomPanel:Awake()
    this = self
    self:Init()
    ...
    this.HomeDrop = this.mingPaiPlayWayNode:Find("Line5/Home/Dropdown"):GetComponent("Dropdown")    --庄家
    ...
    this.ModifyButton = bottom:Find("Button/ModifyButton")
    ...
    -- UIUtil.AddDropdownOptionsByString(this.maxQiangDrop, LYCRuleQiangZhuangList)
...
function ModifyLYCRoomPanel:OnOpened(fromType, funcType, args, isModify, playWayName, rules, advancedData)
    this.isModify = isModify or false
    this.playWayName = playWayName
    this.rules = rules
    LogError("<color=aqua>this.rules</color>", rules)
...
--
function ModifyLYCRoomPanel:OnClosed()
...
---UI监听事件
function ModifyLYCRoomPanel:AddUIListenerEvent()
...
function ModifyLYCRoomPanel.CheckArgsData(fromType, moneyType, args)
    ...
    if this.roomType == RoomType.Lobby then
        ...
        this.lastPlayWayType = LYCPlayType.MingPaiQiangZhuang
...
function ModifyLYCRoomPanel.InitExternalMenu(listToggles)
...
function ModifyLYCRoomPanel.ButtonActiveCtrl()
    LogError("this.isModify", this.isModify)
    UIUtil.SetActive(this.ModifyButton, this.isModify)
    UIUtil.SetActive(this.createRoomBtn, not this.isModify)
end

--检测按钮显示
function ModifyLYCRoomPanel.CheckButtonDisplay()
...
function ModifyLYCRoomPanel.CheckAndUpdateConfigData()
...
function ModifyLYCRoomPanel.RevertLastModifyDisplay()
    LogError("<color=aqua>rules</color>", this.rules)
    this.playerTotalDrop.value = this.GetDropdownIndexOfConfig(LYCRulePlayerNumberConfig, LYCRuleType.GameTotal)
    this.baseCoreDrop.value = this.GetDropdownIndexOfConfig(LYCRuleDiFenConfig, LYCRuleType.BaseScore)
    this.startModelDrop.value = this.GetDropdownIndexOfConfig(LYCRuleStartModelConfig, LYCRuleType.StartModel)
    this.gameTotalDrop.value = this.GetDropdownIndexOfConfig(LYCRuleJuShuConfig, LYCRuleType.JuShu)
    this.maxQiangDrop.value = this.GetDropdownIndexOfConfig(LYCRuleQiangZhuangConfig, LYCRuleType.QZFanBei)
    LogError("this.GetLYCDropDownValue(LYCMaBaoSelect, LYCRuleType.MaBao)", this.GetLYCDropDownValue(LYCRuleType.MaBao))
    this.maBaoDrop.value = this.GetDropdownIndexOfConfig(LYCMaoBaoSelectConfig, LYCRuleType.MaBao)
    --this.zouShuiDrop.value = this.GetLYCDropDownValue(LYCRuleType.ZouShui)
    this.HomeDrop.value = this.GetDropdownIndexOfConfig(ZhuangModeSelectConfig, LYCRuleType.RobZhuang)
    this.PinJuDrop.value = this.GetLYCDropDownValue(LYCRuleType.Tie)
    this.PaiShuDrop.value = this.GetLYCDropDownValue(LYCRuleType.CardCount)
    this.SanPiDrop.value = this.GetDropdownIndexOfConfig(SanPiSelectConfig, LYCRuleType.ThreeBatch)

    --this.laiZiDrop.value = this.GetDropdownIndexOfConfig(LYCRuleLaiZiTypeConfig, LYCRuleType.LaiZi)
    --this.tuiZhuDrop.value = this.GetDropdownIndexOfConfig(LYCRuleBolusConfig, LYCRuleType.TuiZhu)
    --this.fanBeiDrop.value = this.GetDropdownIndexOfConfig(LYCRuleFanBeiRuleConfig, LYCRuleType.FanBeiRule)

    local specialCardTypes = string.split(this.rules[LYCRuleType.SpecialCard], ",")
    this.niuCardType[1].toggle.isOn = table.ContainValue(specialCardTypes, tostring(LYCRuleTeShu[LYCRuleType.BaoZi]))
    this.niuCardType[2].toggle.isOn = table.ContainValue(specialCardTypes, tostring(LYCRuleTeShu[LYCRuleType.TripleYan]))
    this.niuCardType[3].toggle.isOn = table.ContainValue(specialCardTypes, tostring(LYCRuleTeShu[LYCRuleType.DoubleYan]))
end

function ModifyLYCRoomPanel.GetLYCDropDownValue(LYCRuleType)
    return this.rules[LYCRuleType] - 1
end

function ModifyLYCRoomPanel.GetDropdownIndexOfConfig(config, LYCRuleType)
    local index = 0
    for i = 1, #config do
        if config[i] == this.rules[LYCRuleType] then
            index = i
        end
    end
    return index - 1
...
function ModifyLYCRoomPanel.CheckLobbyPlayWayData()
    if this.lobbyPlayWayData == nil then
        ...
        if temp ~= nil then
            ...
            if this.lobbyPlayWayData.lastPlayWayType == nil then
                this.lobbyPlayWayData.lastPlayWayType = LYCPlayType.MingPaiQiangZhuang
            end
...
--玩家总数
function ModifyLYCRoomPanel.OnPlayerTotalDropDown(value)
    this.UpdateConsumeDisplay()
end
--底分
function ModifyLYCRoomPanel.OnBaseCoreDropDown(value)
    --Log('OnBaseCoreDropDown', value)
    --this.CheckRuleValue(LYCRuleType.BaseScore, value)
end

--局数
function ModifyLYCRoomPanel.OnGameTotalDropDown(value)
    this.UpdateConsumeDisplay()
end

--开始模式
function ModifyLYCRoomPanel.OnStartModelDropDown(value)
    --this.CheckRuleValue(LYCRuleType.StartModel, value)
end

function ModifyLYCRoomPanel.OnPayDropDown(value)
    --this.CheckRuleValue(LYCRuleType.PayType, value)
end

--推注
function ModifyLYCRoomPanel.OnTuiZhuDropDown(value)
    --this.CheckRuleValue(LYCRuleType.TuiZhu, value)
end
--最大抢庄
function ModifyLYCRoomPanel.OnMaxQiangDropDown(value)
    --this.CheckRuleValue(LYCRuleType.MaxQiangZhuang, value)
end

--癞子
function ModifyLYCRoomPanel.OnLaiZiDropDown(value)
    --this.CheckRuleValue(LYCRuleType.LaiZi, value)
end
--翻倍规则
function ModifyLYCRoomPanel.OnFanBeiDropDown(value)
    --this.CheckRuleValue(LYCRuleType.FanBeiRule, value)
    this.UpdateNiuCardTxt()
end

--通过Dropdown索引获取值
function ModifyLYCRoomPanel.CheckRuleValue(ruleType, index)
    local tempValue = 0
...
--检测输入值
function ModifyLYCRoomPanel.CheckInputValue(value)
...
--纠正Dropdown中的索引
function ModifyLYCRoomPanel.ExchangeDropValueByRule(ruleType, ruleValue)
    if ruleType == LYCRuleType.TuiZhu then
        this.tuiZhuDrop.value = LYCConfig.GetConfigIndex(LYCRuleBolusConfig, ruleValue)
    elseif ruleType == LYCRuleType.GameTotal then
    ...
    elseif ruleType == LYCRuleType.QZFanBei then
        this.maxQiangDrop.value = LYCConfig.GetConfigIndex(LYCRuleQiangZhuangConfig, ruleValue)
    ...
    elseif ruleType == LYCRuleType.PayType then
        this.zhifuDrop.value = LYCConfig.GetConfigIndex(LYCRulePayConfig, ruleValue)
    end
end

--特殊牌型按钮
function ModifyLYCRoomPanel.OnSpecialCardBtnClick()
...
--高级选项按钮
function ModifyLYCRoomPanel.OnHighOptionBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
...
function ModifyLYCRoomPanel.OnCloseOptionBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
    UIUtil.SetActive(this.optionGroupNode, false)
...
function ModifyLYCRoomPanel.OnCloseSpecialBtnClick()
    UIUtil.SetActive(this.niuCardNode, false)
    UIUtil.SetActive(this.optionGroupNode, false)
...
--判断牛型是否全选
function ModifyLYCRoomPanel.CheckSpecialIsAllSlected()
...
--判断高级选项是否全选
function ModifyLYCRoomPanel.CheckOptionAllSlected()
    local isAllSlected = true
    for i = 1, 4 do
...
--特殊类型选择
function ModifyLYCRoomPanel.OnNiuTypeValueChanged(isOn, listener)
...
--特殊牌型的全选
function ModifyLYCRoomPanel.AllNiuTypeValueChanged(isOn, listener)
...
--高级选项的选择
function ModifyLYCRoomPanel.OnOptionValueChanged(isOn, listener)
    if this.isOpend then
        this.CheckOptionAllSlected()
        this.UpdateOptionTxt()
    end
...
--高级选项的全选
function ModifyLYCRoomPanel.AllOptionValueChanged(isOn, listener)
    if this.isNotTogglePress then
        return
    end
    for i = 1, 4 do
...
--更新高级选项文本
function ModifyLYCRoomPanel.UpdateOptionTxt()
    local tempStr = ""
    for i = 1, 4 do
...
--更新牛型显示文本
function ModifyLYCRoomPanel.UpdateNiuCardTxt()
    local tempStr = ""
...
--保存到本地存储中
function ModifyLYCRoomPanel.SaveLobbyPlayWayConfigData()
    if this.lobbyPlayWayData ~= nil then
        local temp = {}
...
--保存配置数据
function ModifyLYCRoomPanel.SavePlayWayConfigData()
...
--无规则时设置默认规则界面
function ModifyLYCRoomPanel.SetDefaultRuleUI()
...
--获取当前选择的UI上的默认字段
function ModifyLYCRoomPanel.GetPlayWayRuleDataAtUI()
    --玩法规则对象
    ...
    --playWayRuleData[LYCRuleType.MaxQiangZhuang] = this.CheckRuleValue(LYCRuleType.MaxQiangZhuang, this.maxQiangDrop.value)
    ...
    playWayRuleData[LYCRuleType.QZFenShu] = 0
    ...
    LogError(">> ModifyLYCRoomPanel.GetPlayWayRuleDataAtUI > ", playWayRuleData)
    return playWayRuleData
end

function ModifyLYCRoomPanel.GetTeShuCardType(type, toggle, str)
...
function ModifyLYCRoomPanel.GetHighOptionCardType(type, toggle, str)
    local isOne = this.GetToggleRuleValue(toggle.toggle.isOn) == 1
    if isOne then
        if string.IsNullOrEmpty(str) then
...
function ModifyLYCRoomPanel.GetToggleRuleValue(isOn)
    if isOn then
        return 1
    else
        return 0
...
function ModifyLYCRoomPanel.SetRuleUI(rules)
    if rules == nil then
...
--获取规则值
function ModifyLYCRoomPanel.GetRuleValue(rules, type)
...
function ModifyLYCRoomPanel.UpdateDropDown(ruleValue)
    local optionDatas = {}
    for i = 1, 3 do
...
function ModifyLYCRoomPanel.OnCreateBtnClick()
...
function ModifyLYCRoomPanel.OnCreateAlert()
    this.HandleCreateRoom()
end

--高级设置按钮
function ModifyLYCRoomPanel.OnAdvancedBtnClick()
...
local tempPlayWayType = LYCPlayType.MingPaiQiangZhuang
...
local wins = ""
local consts = ""
function ModifyLYCRoomPanel.HandleCreateRoom()
    tempRuleDatas = nil
    --只有大厅创建房间才进行规则存储
    ...
    if this.moneyType == MoneyType.Gold then
        ...
        tempRuleDatas[LYCRuleType.QZFenShu] = tempRobLimit
...
--保存一键配置
function ModifyLYCRoomPanel.OnSaveBtnClick()
    if this.roomType == RoomType.Club then
        Alert.Prompt("确定保存一键开房配置信息？", this.OnSaveAlert)
    end
end

function ModifyLYCRoomPanel.OnSaveAlert()
...
--删除一键配置
function ModifyLYCRoomPanel.OnDeleteBtnClick()
    if this.roomType == RoomType.Club then
        Alert.Prompt("确定删除一键开房配置？", this.OnDeleteAlert)
    end
end

function ModifyLYCRoomPanel.OnDeleteAlert()
    ClubData.SendRemoveYjpzRule(GameType.LYC, this.lastPlayWayType)
end


--菜单按钮点击
function ModifyLYCRoomPanel.OnPlayWayMenuValueChanged(isOn, listener)
    if isOn then
...
--
function ModifyLYCRoomPanel.UpdateConsumeDisplay()
...
--获取label字体的颜色
function ModifyLYCRoomPanel.SetLabelColorTxt(toggle)
...
--更新创建房间高级设置
function ModifyLYCRoomPanel.OnUpdateCreateRoomAdvanced(data)
