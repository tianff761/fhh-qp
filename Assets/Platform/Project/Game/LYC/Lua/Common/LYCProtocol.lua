--加入房间信息
LYCAction.LYC_STC_JoinRoom_Info = {
    state = 1, --状态(number)
    ...
    rule = {}
    --faceType = 0,--分配规则 0/1 所有人分配/赢家分配
}

--通知游戏是否可以开始
LYCAction.LYC_STC_Start_State = {}

--通知房间变化
LYCAction.LYC_STC_RoomState = {
    state = 0, --房间状态(number)
...
--更新玩家信息
LYCAction.LYC_STC_Update_Player_Info = {
    userId = 0, --玩家id(number)
...
--操作回复
LYCAction.LYC_STC_Operate = {}

--广播玩家操作
LYCAction.LYC_STC_B_Operate = {
...
--广播某个玩家亮牌
LYCAction.LYC_STC_B_FlipCard = {
    playerId = 0, --玩家id(number)
    midCard = {}, --手牌(table){101,201,301,504,1001}
    lycCard = {}, --组成炮的两张牌(table){101,102}
    point = 0, --炮的类型(number)
    lastCard = 0, --最后一张牌(number)
}

--请求提示
LYCAction.LYC_CTS_GetTipCard = {

}

--请求提示回复
LYCAction.LYC_STC_GetTipCard = {
    midCard = {}, --手牌(table)
    lycCard = {}, --组成炮的两张牌(table)
    point = 0, --炮的类型(number)
}

--广播小结算
LYCAction.LYC_STC_B_XiaoJie = {
